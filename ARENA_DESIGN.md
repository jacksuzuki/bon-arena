...
host が固めるのは「何が求められているか」（ユーザーの意図・決定事項・外から観察できる合格基準）まで。
調査（コードの深い読解、ツールや API の実機検証）と設計（ファイル、構造、内部の命名、方式）は runner に残す。
Best-of-N の価値は独立した候補のばらつきにあり、host が先に調べて決めたことは誤りも含めて全候補に共有されるため。

1. 要求を理解する。コードは「依頼がこのリポジトリで何を意味するか・どこが曖昧か」が分かる範囲だけ読む（読み取りのみ）
2. 「何を」について runner が推測することになる点を洗い出す（範囲、エッジケース、公開 API とユーザーに見える名前、互換性）。
   「どう作るか」の選択は gap ではなく "Open to the implementer" に列挙する
3. 慣習や妥当な既定値から決められるものは host が決める
4. 残りだけをユーザーに聞く（選択肢と推奨値付き、まとめて、最大 2 ラウンド）
5. 仕様（Goal / Context / Scope / Requirements / Acceptance criteria / Constraints / Verification / Decisions / Open to the implementer / Notes (unverified)）を書く
   - 各行を「別の同じくらい良い実装がこの行に違反するか」で点検し、違反するのにユーザーも公開契約も求めていない行は削るか Open に移す
   - 合格基準は観察できる振る舞いで書き、内部構造（関数名、引数の並び、ファイル配置）を検査しない
   - host が得た実装上の知見は Notes (unverified) に分ける（runner には「確かめること」と伝える）か、書かない
