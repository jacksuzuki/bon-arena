...
2〜3体のコーディングエージェントに**同じタスク**を**別々の git worktree** で実装させ、成果物（diff の統計、テスト、lint、typecheck）を比較して、採用するものを選べるツールです。
...
> **セキュリティ:** 組み込み runner はフル権限で動きます（承認を求めるものも、閉じ込めるものもありません）。信頼できるコードで使うか、コンテナや VM の中で動かしてください。[セキュリティ](#セキュリティ)を参照。
...
Claude Code は Player 1 / Player 2 と任意の Player 3 を尋ねたあと、何かを起動する前に**タスクを洗練**します。**あなたが何を求めているか**について runner が推測せざるを得ない点（範囲、エッジケース、ユーザーに見える名前、互換性）を洗い出し、自分で決められない点だけをあなたに質問し、仕様（目的、文脈、範囲、要件、外から観察できる振る舞いとしての受け入れ条件、制約、検証、決定事項、実装者に委ねる点）を書きます。ホストは意図的にそこで止まります。コードの深い調査と設計は runner が競う部分であり、ホストが先に済ませたことは誤りも含めて全候補に共有されてしまうからです。ホストがたまたま得た実装上の知見は短い「Notes (unverified)」に分けて書かれ、runner には「従うのではなく確かめること」と伝えられます。あなたが承認または編集してから、各プレイヤーがその仕様を持って隔離された worktree で起動されます。runner はヘッドレスで質問できないため、この工程が「runner ごとに別々の推測をする」ことを防ぎます。元の依頼はセッションに保存され、`arena compare` で仕様と並べて表示されます。
...
runner の完了後、Claude Code は待機し、検証を実行してサマリーを表示し、**必ず先に比較を提示**します。事実、観点ごとの判断、推奨ベース、もう一方の候補の優れた点です。そのうえで次の行動を尋ねます。推奨は **Synthesize**（強い候補をベースにし、その worktree の中でもう一方の長所を取り込み、再検証して候補ブランチにコミット）です。どちらかの候補をそのまま採用することもできます。マージを尋ねる前に、Claude Code は**すべての runner に最終版をレビューさせます**（`arena review`）。各 runner は自分の会話を読み取り専用で再開し、最終版の diff を見て、verdict と所見を返します。Claude Code は所見を検証し、認めたものを直し、却下したものは理由と一緒に提示します。あなたのブランチへのマージ（`arena adopt`）はあなたが指示したときだけ行われ、push は決して行いません。
...
arena run --players claude,codex,agy --task "API にレート制限を追加"   # 3体。同じ runner の重複も可（claude,claude → claude, claude-2）
# あるいは段階的に
...
全プレイヤーは同じプロンプトを受け取ります。共通の arena ルール（現在の worktree 内だけで作業する、タスクを完遂する、テストを実行する、push しない）に続けてタスクをそのまま渡します。refined モードではタスクはユーザーと合意した仕様で、ルールに「この仕様は確定済み」が加わります。元の依頼はプロンプトに含まれないので、runner が再解釈することはありません。
...
ヘッドレス実行では権限の確認に答えられないため、組み込み runner はすべて権限チェックをスキップし、何にも閉じ込められずに動きます（[セキュリティ](#セキュリティ)を参照）。各 runner は detached な supervisor プロセスに監視され、exit code が記録されるので、`arena` コマンドは終了してあとから戻れます（`arena wait`、`arena status`）。`arena stop` はプロセスグループ全体を終了します。
...
**組み込み runner はフル権限で動きます。承認を求めるものも、閉じ込めるものもありません。** runner はあなたのユーザーアカウントでできることを何でもできます。worktree の外のファイル、認証情報、ネットワークも含みます。専用の worktree は候補どうしの変更を分けるためのもので、セキュリティ境界ではありません。プロンプトの規則（「worktree の中だけで作業する」「push しない」）は指示であって、強制ではありません。
...
Antigravity は仕組みの上でだけ例外で、結果は同じです。`agy` にはターミナルサンドボックスがあり、起動フラグでは無効にできません。ヘッドレス実行でも `~/.gemini/antigravity-cli/settings.json` の `enableTerminalSandbox` に従います。有効な場合、`agy` は各コマンドをまずサンドボックス内で実行しますが、ブロックされたコマンドをサンドボックス外で再実行でき、その要求も `--dangerously-skip-permissions` が承認します。つまり最初の失敗を挟むだけで、他の runner と同じ範囲に届きます。Arena があなたの `agy` の設定を書き換えることはありません。
...
勝者の自動決定、相互レビュー、`/arena` での4体以上のプレイヤー、トーナメント、クラウド実行、Web UI、Superset アダプタ、Orca への runner 実行委譲、MCP、PR 作成、自動マージ、コスト計測。
