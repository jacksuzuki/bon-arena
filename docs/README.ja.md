...
Claude Code は Player 1 / Player 2 を尋ねたあと、何かを起動する前に**タスクを洗練**します。**あなたが何を求めているか**について runner が推測せざるを得ない点（範囲、エッジケース、ユーザーに見える名前、互換性）を洗い出し、自分で決められない点だけをあなたに質問し、仕様（目的、文脈、範囲、要件、外から観察できる振る舞いとしての受け入れ条件、制約、検証、決定事項、実装者に委ねる点）を書きます。ホストは意図的にそこで止まります。コードの深い調査と設計は runner が競う部分であり、ホストが先に済ませたことは誤りも含めて全候補に共有されてしまうからです。ホストがたまたま得た実装上の知見は短い「Notes (unverified)」に分けて書かれ、runner には「従うのではなく確かめること」と伝えられます。あなたが承認または編集してから、両プレイヤーがその仕様を持って隔離された worktree で起動されます。runner はヘッドレスで質問できないため、この工程が「2者が別々の推測をする」ことを防ぎます。元の依頼はセッションに保存され、`arena compare` で仕様と並べて表示されます。
