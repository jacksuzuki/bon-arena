...
import type { SupervisorSpec } from "./supervisor.ts"
...
export function runnerEnvironment(extra: Record<string, string> = {}): Record<string, string> {
 ...
 */
export function runForeground(opts: ForegroundOptions): Promise<ForegroundResult> {
  mkdirSync(dirname(opts.stdoutPath), { recursive: true })
...
export function readExitCode(exitCodePath: string): number | null {
  if (!existsSync(exitCodePath)) return null
