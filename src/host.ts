...
export type Harness = "claude-code" | "unknown"
...
export interface HostInfo {
  ...
  model?: string
  effort?: string
  /** Where each value came from, for transparency. */
  sources: { model?: string; effort?: string }
  /** 0 = below minimum, 1 = meets minimum, 2 = recommended. -1 when unknown. */
  modelTier: number
  effortTier: number
  warnings: string[]
...
export const MODEL_TIERS: Array<{ pattern: RegExp; tier: number; label: string }> = [
...
export const EFFORT_RANK: Record<string, number> = { low: 0, medium: 1, high: 2, xhigh: 3, max: 4 }
...
export function modelTier(model: string | undefined): number {
  if (!model) return -1
  for (const t of MODEL_TIERS) if (t.pattern.test(model)) return t.tier
  return -1
}
...
export function findTranscript(sessionId: string, cwd: string, configDir = claudeConfigDir()): string | null {
...
export function readTranscriptTail(path: string, maxBytes = 512 * 1024): { model?: string; effort?: string } {
  ...
  for (const line of tail.split("\n").reverse()) {
    ...
    try {
      const rec = JSON.parse(line) as { type?: string; effort?: string; message?: { model?: string } }
...
function readSettings(configDir: string): { model?: string; effortLevel?: string } {
  const p = join(configDir, "settings.json")
  if (!existsSync(p)) return {}
  try {
    const j = JSON.parse(readFileSync(p, "utf8")) as { model?: string; effortLevel?: string }
...
export function rateHost(info: Omit<HostInfo, "modelTier" | "effortTier" | "warnings" | "recommendation">): HostInfo {
  const mt = modelTier(info.model)
  const et = effortRank(info.effort)
  const warnings: string[] = []
  if (info.harness === "claude-code") {
    if (info.model === undefined) warnings.push("host model could not be detected")
    else if (mt < 0) warnings.push(`host model "${info.model}" is not recognised; make sure it is Opus-class or better`)
    else if (mt < MIN_MODEL_TIER) warnings.push(`host model "${info.model}" is below the minimum (Opus 5); the compare/refine/synthesize steps need a stronger model`)
    if (info.effort === undefined) warnings.push("host effort could not be detected")
    else if (et < 0) warnings.push(`host effort "${info.effort}" is not recognised`)
    else if (et < MIN_EFFORT_RANK) warnings.push(`host effort "${info.effort}" is below the minimum (medium); use /effort high for arena work`)
  }
  return { ...info, modelTier: mt, effortTier: et, warnings, recommendation: RECOMMENDATION }
}

export function detectHost(cwd = process.cwd(), env: NodeJS.ProcessEnv = process.env): HostInfo {
  ...
  const sources: HostInfo["sources"] = {}
  let model: string | undefined
  let effort: string | undefined
...
export function describeHost(h: HostInfo): string[] {
  if (h.harness !== "claude-code") return ["harness    (not Claude Code; model/effort not detected)"]
  const lines = [
  ...
  if (h.warnings.length) lines.push(`recommended: ${h.recommendation}`)
  return lines
}
