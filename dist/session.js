...
export function newArenaId(now = new Date()) {
...
export function resolveSessionId(idOrPrefix) {
    ...
    if (idOrPrefix === "latest" || idOrPrefix === "last") {
        const latest = all[0];
        if (!latest)
            throw new Error("No arena sessions found");
        return latest.id;
    }
    ...
    if (matches.length === 1)
        return matches[0].id;
    if (matches.length === 0)
        throw new Error(`Arena session not found: ${idOrPrefix}`);
    throw new Error(`Ambiguous session id "${idOrPrefix}": ${matches.map((s) => s.id).join(", ")}`);
}
export function findPlayer(session, idOrRunner) {
    const p = session.players.find((x) => x.id === idOrRunner) ??
