...
import { gitTry, inspectRepository } from "./git/repository.js";
import { draftsDir } from "./paths.js";
import { newArenaId } from "./session.js";
...
export function defaultTaskMode(repoRoot) {
...
export function repositoryLayout(root, max = 40) {
    const out = gitTry(root, ["ls-tree", "--name-only", "HEAD"]);
    if (out.exitCode !== 0)
        return [];
    const dirs = new Set(gitTry(root, ["ls-tree", "-d", "--name-only", "HEAD"])
        .stdout.split("\n")
        .map((l) => l.trim())
        .filter(Boolean));
...
export function saveTaskDraft(task, id = newArenaId()) {
    const dir = draftsDir();
...
export function buildRefineContext(opts) {
...
export function renderRefineBrief(ctx) {
    const out = [];
